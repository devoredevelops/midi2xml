...
NOTE_TO_PITCH = {"a": 9, "b": 11, "c": 12, "d": 14, "e": 16, "f": 17, "g": 19}
...
RM_TO_PITCH = {"vii": 11, "iii": 4, "vi": 9, "iv": 5, "ii": 2, "i": 0, "v": 7}

ACC_TO_SHIFT = {"b": -1, "#": 1}

SCALE_DEGREE_SHIFT = {1: 0, 2: 2, 3: 4, 4: 5, 5: 7, 6: 9, 7: 11}
        ...
        for i_, curr_note in enumerate(curr_chord):
            ...
            if i_ == 0:
                ...
                if abs(curr_note - prev_note) > 7:
                    if curr_note < prev_note and abs(curr_note + 12 - prev_note) < abs(
                        curr_note - prev_note
                    ):
                        best_voicing = curr_note + 12
                    elif curr_note > prev_note and abs(
                        curr_note - 12 - prev_note
                    ) < abs(curr_note - prev_note):
                        best_voicing = curr_note - 12
                else:
            ...
            if curr_note <= best_neighbor:
                best_voicing = (
                    curr_note + math.floor((best_neighbor - curr_note + 6) / 12) * 12
                )
            else:
                best_voicing = (
                    curr_note + math.ceil((best_neighbor - curr_note - 6) / 12) * 12
                )

            best_voicing = (
                best_voicing
                if (abs(best_voicing - center) <= 8 or allowance > 2)
                else curr_note
            )
            voiced_chord.append(best_voicing)
...
class Chord:
    def __init__(self, string):
        ...
        for i in range(0, len(string) - 1):
            ...
            root_found = not root_added and (
                curr_char in notes + rms + accs and not next_char in rms + accs
            )
            mode_found = False
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="ezchord - convert complex chord names to midi notes"
    )
    parser.add_argument(
        "chords",
        type=str,
        nargs="+",
        help="Sequence of chord names (e.g. C C7 F Fmin6 C/G G7 C)\n'-' continues the previous chord\n'nc' inserts a rest",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Duration of each chord (default 2)",
    )
    parser.add_argument(
        "-s",
        "--subdivide",
        type=int,
        default=1,
        help="Subdivide chord's duration (default 1)",
    )
    parser.add_argument(
        "-v", "--velocity", type=int, default=100, help="Velocity (default 100)"
    )
    parser.add_argument(
        "-O", "--octave", type=int, default=4, help="Octave (default 4)"
    )
    parser.add_argument("-o", "--output", type=str, help="Output file path")
    parser.add_argument(
        "--voice", action="store_true", help="Attempts to give chords a better voicing"
    )
        ...
        for pitch in chord:
            for d in range(0, args.subdivide):
                MIDI.addNote(
                    ...
                    0,
                    pitch,
                    i * args.duration + d * (args.duration / args.subdivide),
                    args.duration / args.subdivide,
                    args.velocity,
                )
